...
export const probeMetadata = (probeIpLimit: ProbeIpLimit, probeOverride: ProbeOverride) => errorHandler(async (socket: ServerSocket, next: (error?: Error) => void) => {
	const clientIp = getProbeIp(socket);

	try {
		const probe = await buildProbe(socket, probeIpLimit);
		const serverProbe = { ...probe, location: probeOverride.getUpdatedLocation(probe) };
		await probeIpLimit.verifyAsnLimit(serverProbe);
		socket.data.probe = probe;
		next();
	} catch (error: unknown) {
