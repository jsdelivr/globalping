...
describe('Error handler middleware', () => {
	...
	it('should handle http errors', async () => {
		...
		expect(ctx.body).to.deep.equal({ error: { message: 'bad request', type: 'api_error' }, links: { documentation } });
	});
	...
	it('should handle custom errors', async () => {
		...
		expect(ctx.body).to.deep.equal({ error: { message: 'Internal Server Error.', type: 'api_error' }, links: { documentation } });
	});
});
