...
export const parseMeasurementId = (id: string): ParsedMeasurementId => {
	...
	if (version !== 2) {
		throw new Error('Unsupported measurement ID version');
	}
...
function parseMeasurementIdV2 (id: string): ParsedMeasurementId {
	if (id.length < 24) {
		throw new Error('Measurement ID must be at least 24 characters long');
	}
	...
	if (!STORAGE_STRATEGY_INVERTED[storageStrategy]) {
		throw new Error('Unknown storage strategy');
	}
