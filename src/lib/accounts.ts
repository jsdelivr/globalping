...
export type ResolvedAccount = { id: string; role: AccountRole };
...
export const getAccountRole = async (accountId: string, userId: string): Promise<ResolvedAccount | null> => {
	...
	const account = await dashboardClient(ACCOUNTS_TABLE)
		...
		.first<{ id: string; role: AccountRole } | undefined>(
			`${ACCOUNTS_TABLE}.id`,
			dashboardClient.raw(`IF(?? = ?, 'owner', ??) AS role`, [ `${ACCOUNTS_TABLE}.user`, userId, `${MEMBERS_TABLE}.role` ]),
		);

	const role = account ? { id: account.id, role: account.role } : null;
